...
const localWorkers = (await readdir(assetsUrl)).filter((name) => name.startsWith("pdf.worker-"));
const runtimeAssetDirs = ["pdfjs/cmaps", "pdfjs/wasm", "pdfjs/standard_fonts"];

if (!localEntry || localWorkers.length !== 1) {
  throw new Error("Local Reader build is missing its entry or unique PDF worker");
}
...
const runtimeAssetSamples = await Promise.all(runtimeAssetDirs.map(async (directory) => {
  ...
  if (!files.length) throw new Error(`Local Reader build has no ${directory} assets`);
...
for (let attempt = 1; attempt <= attempts; attempt += 1) {
  try {
    ...
    if (!entry.includes(expectedWorker)) {
      throw new Error(`Entry does not reference ${expectedWorker}`);
    }
    ...
    console.log(`Reader deployment verified: ${localEntry} -> ${expectedWorker} and PDF runtime assets (${workerType})`);
    process.exit(0);
  } catch (error) {
...
throw new Error(`Reader deployment verification failed after ${attempts} attempts: ${lastError.message}`);

# JOJO Web regression test strategy

JOJO Web uses two complementary test layers so interaction coverage can grow without making pull requests slow.

## Fast UI and logic tests (Vitest + jsdom)

These run on every Web pull request and should contain most regression cases.
...
```bash
pnpm --filter @jojo/web... typecheck
pnpm --filter @jojo/web... test
pnpm --filter @jojo/web build
pnpm --filter @jojo/web verify:build
pnpm --filter @jojo/web exec playwright test
```

The pull-request workflow runs unit/build checks and E2E in parallel. The expected budget is:

- Web and shared UI Vitest suites: under 15 seconds on CI;
- Playwright execution after browser installation: under 45 seconds;
- total Web CI wall time: bounded by the eight-minute job timeout, normally around two minutes or less.

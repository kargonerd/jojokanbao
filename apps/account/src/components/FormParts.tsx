...
export function AuthField({
  ...
  return (
    <label className="block">
      <span className="mb-2 flex items-baseline justify-between gap-3 text-xs font-bold tracking-[0.08em]">
        {label}
        {hint ? <span className="font-normal tracking-normal text-muted">{hint}</span> : null}
      </span>
      <input
        {...props}
...
export function PasswordField(props: Omit<InputHTMLAttributes<HTMLInputElement>, "type"> & { label: string; hint?: ReactNode }) {

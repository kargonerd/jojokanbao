...
create or replace function private.annotation_snapshot(p_annotation_id uuid, p_public_mark_threshold integer default 2)
returns jsonb
language sql
stable
security definer
...
begin
  ...
  return (
    with page as materialized (
      ...
      select page.id, private.annotation_snapshot(page.id, public_threshold) as snapshot
      from page
    )
...
begin
  ...
  return (
    with mine as (
      ...
      select page.id, private.annotation_snapshot(page.id, 2) as snapshot
      from page
    )
    select coalesce(jsonb_agg(
      snapshots.snapshot || jsonb_build_object('comments', coalesce((

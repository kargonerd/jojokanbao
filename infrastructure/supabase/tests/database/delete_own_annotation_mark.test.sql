...
select extensions.is(private.delete_my_annotation_mark(2, '00000000-0000-4000-9000-000000009210')->'thread', 'null'::jsonb, 'removing my mark hides an anchor below the sharing threshold');
...
select extensions.is(jsonb_array_length(private.get_annotation_threads(2, 'book', 'delete-mark-book', 'chapter')), 0, 'subsequent loads agree with the deletion result');
select private.delete_my_annotation_mark(2, '00000000-0000-4000-9000-000000009210');
...
select extensions.is(private.get_annotation_threads(2, 'book', 'delete-mark-book', 'chapter') #>> '{0,underlinedByMe}', 'true', 'the other reader still sees their own underline');
insert into public.annotation_comments(id, annotation_id, user_id, body, visibility) values
...
select extensions.is(private.delete_my_annotation_mark(2, '00000000-0000-4000-9000-000000009210') #>> '{thread,underlineCount}', '0', 'public thoughts survive removal of the final underline with a zero count');
select extensions.is(private.get_annotation_threads(2, 'book', 'delete-mark-book', 'chapter') #>> '{0,underlinedByMe}', 'false', 'deleted marks stay removed on reload');
select extensions.is(private.get_annotation_threads(2, 'book', 'delete-mark-book', 'chapter') #>> '{0,comments,0,likeCount}', '1', 'deletion preserves public thoughts and their likes');
select extensions.is(jsonb_array_length(private.get_annotation_threads(2, 'book', 'delete-mark-book', 'chapter') #> '{0,comments}'), 1, 'deletion does not expose another reader’s private thought');
select extensions.is(private.delete_my_annotation_mark(2, '00000000-0000-4000-9000-000000009210') #>> '{thread,underlinedByMe}', 'false', 'retrying a deletion returns the current shared state');
select extensions.is(private.delete_my_annotation_mark(2, '00000000-0000-4000-9000-000000009299')->'thread', 'null'::jsonb, 'deleting a missing anchor is harmless');

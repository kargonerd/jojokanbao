...
select extensions.is(private.feature_flag_snapshot('library.bookshelf')->>'rolloutProvider', 'retired', 'bookshelf rollout is retired');
select extensions.is(private.feature_flag_snapshot('reader.annotations')->>'rolloutProvider', 'retired', 'annotations rollout is retired');
select extensions.is(private.feature_flag_snapshot('reader.speech')->>'rolloutProvider', 'retired', 'speech rollout is retired');
...
-- A legacy off decision must not reject a current authenticated client. Ownership still applies.
...
select extensions.is(private.require_annotation_reader(), (select owner_id from posthog_flag_test_state), 'annotation RPCs accept an authenticated client');

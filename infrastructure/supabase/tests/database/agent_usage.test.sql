...
select extensions.throws_ok(
  $$select public.acquire_agent_usage('bad', '00000000-0000-4000-8000-000000000081', extensions.gen_random_uuid(), 3,100,300)$$,
  '42501', 'Operator token is invalid', 'forged reservations are rejected');
select extensions.throws_ok(
  $$select public.release_agent_usage('bad', '00000000-0000-4000-8000-000000000081', extensions.gen_random_uuid())$$,
  '42501', 'Operator token is invalid', 'forged releases are rejected');
select extensions.is((select count(*)::integer from private.agent_usage_state), 0, 'unauthorized calls create no counters');
select extensions.is(
  public.acquire_agent_usage(repeat('u',32), '00000000-0000-4000-8000-000000000081', '00000000-0000-4000-8000-000000000091', 3,100,300)->>'allowed',
  'true', 'first request obtains a lease');
select extensions.is(
  public.acquire_agent_usage(repeat('u',32), '00000000-0000-4000-8000-000000000081', '00000000-0000-4000-8000-000000000092', 3,100,300)->>'reason',
...
select extensions.is(
  public.acquire_agent_usage(repeat('u',32), '00000000-0000-4000-8000-000000000082', '00000000-0000-4000-8000-000000000092', 3,100,300)->>'allowed',
...
select extensions.is(
  public.acquire_agent_usage(repeat('u',32), '00000000-0000-4000-8000-000000000081', extensions.gen_random_uuid(), 1,100,120)->>'reason',
...
) from (select public.acquire_agent_usage(repeat('u',32), '00000000-0000-4000-8000-000000000081', '00000000-0000-4000-8000-000000000093', 1,100,120) as decision) admitted;
select public.release_agent_usage(repeat('u',32), '00000000-0000-4000-8000-000000000081', '00000000-0000-4000-8000-000000000093');
select extensions.is(
  public.acquire_agent_usage(repeat('u',32), '00000000-0000-4000-8000-000000000081', extensions.gen_random_uuid(), 1,2,120)->>'reason',
  'daily', 'daily allowance cannot be bypassed by waiting for a minute');
select extensions.ok(
  (public.acquire_agent_usage(repeat('u',32), '00000000-0000-4000-8000-000000000081', extensions.gen_random_uuid(), 1,2,120)->>'retryAfter')::integer between 1 and 86400,
...
select extensions.is(
  public.acquire_agent_usage(repeat('u',32), '00000000-0000-4000-8000-000000000081', '00000000-0000-4000-8000-000000000094', 1,2,120)->>'allowed',
...
select extensions.is(
  public.acquire_agent_usage(repeat('u',32), '00000000-0000-4000-8000-000000000081', '00000000-0000-4000-8000-000000000095', 1,2,120)->>'allowed',
...
) from (select public.acquire_agent_usage(repeat('u',32), '00000000-0000-4000-8000-000000000081', '00000000-0000-4000-8000-000000000096', 3,100,300) as decision) admitted;

select extensions.throws_ok(format(
  'select public.acquire_agent_usage(repeat(''u'',32), ''00000000-0000-4000-8000-000000000081'', extensions.gen_random_uuid(), %s, %s, %s)',

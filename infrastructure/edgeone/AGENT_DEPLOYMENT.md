...
┌──────────────────────┐             ┌──────────────────────────┐
│ Web + Python API     │ ── HTTPS ─▶ │ /gateway/query proxy    │
│ JOJO/Supabase 登录   │             │ /jojo   Makers Agent     │
└──────────────────────┘             │ /gateway/credentials     │
                                     └──────────────────────────┘
```

- `/gateway/query` 处理浏览器 CORS 预检，为请求添加短时 HMAC 服务签名，再把 SSE
  请求转给同项目 `/jojo`。
...
VITE_AGENT_API_URL=https://agent-global.jojokanbao.cn/gateway/query
...
Agent 运行环境中。`/gateway/query` 使用它对请求方法、会话 ID、规范化请求体、60 秒

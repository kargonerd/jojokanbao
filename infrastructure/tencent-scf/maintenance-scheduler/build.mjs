...
await build({ entryPoints: [new URL("./index.ts", import.meta.url).pathname.replace(/^\/([A-Z]:)/, "$1")],
  outfile: `${here}/dist/index.js`, platform: "node", target: "node20", format: "cjs", bundle: true,
  sourcemap: false, legalComments: "none" });
const bytes = await readFile(`${here}/dist/index.js`);
const metadata = { gitCommit: execFileSync("git", ["rev-parse", "HEAD"], { cwd: here, encoding: "utf8" }).trim(),
  sourceSha256: createHash("sha256").update(bytes).digest("hex"), bytes: bytes.length };

...
def create_app() -> FastAPI:
    app = FastAPI(title="jojo jiuwen api", version="0.1.0")
    app.add_middleware(
        CORSMiddleware,
...
def run() -> None:
    ...
    uvicorn.run("jojo_jiuwen_api.app:app", host=settings.host, port=settings.port, reload=True, reload_dirs=[str(service_dir)])

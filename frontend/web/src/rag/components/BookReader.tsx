...
export function BookReader({
  ...
  const previousPage = useCallback(() => {
    if (pageMetrics.page > 0) goToPage(pageMetrics.page - 1);
    else if (previousChapter) chooseChapter(previousChapter.id, "end");
  }, [chooseChapter, goToPage, pageMetrics.page, previousChapter]);

  const nextPage = useCallback(() => {
    if (pageMetrics.page < pageMetrics.spreads - 1) goToPage(pageMetrics.page + 1);
    else if (nextChapter) chooseChapter(nextChapter.id);
  }, [chooseChapter, goToPage, nextChapter, pageMetrics.page, pageMetrics.spreads]);
  ...
  const chapterNavigation = !contentLoading && <nav aria-label="章节导航" className="mt-20 grid grid-cols-2 border-t border-rule pt-8 font-sans text-xs [break-inside:avoid]">
    <button type="button" disabled={!previousChapter} onClick={() => chooseChapter(previousChapter?.id, "end")} className="border-0 bg-transparent py-4 pr-4 text-left text-current cursor-pointer disabled:cursor-default disabled:opacity-30"><span className="mb-1 block text-muted">上一章</span>{previousChapter?.title ?? "已经是第一章"}</button>
    <button type="button" disabled={!nextChapter} onClick={() => chooseChapter(nextChapter?.id)} className="border-0 border-l border-rule bg-transparent py-4 pl-4 text-right text-current cursor-pointer disabled:cursor-default disabled:opacity-30"><span className="mb-1 block text-muted">下一章</span>{nextChapter?.title ?? "已经是最后一章"}</button>
  </nav>;
  ...
  return <div className={`h-screen overflow-hidden ${shellClass}`}>
    <nav aria-label="阅读工具" className="fixed left-5 top-1/2 z-30 hidden -translate-y-1/2 flex-col gap-2 md:flex">
      <Link to={backHref} className={`${controlClass} no-underline text-current`} aria-label="返回问答" title="返回问答">←</Link>
      ...
      <div className={`relative h-24 w-12 border ${theme === "dark" ? "border-[#444844]" : "border-[#d2d3ce]"}`} aria-label={`全书已读 ${bookProgress}%`} title={`全书已读 ${bookProgress}%`}>
        <div className="absolute inset-x-0 bottom-0 bg-red transition-[height] duration-200" style={{ height: `${bookProgress}%` }} />
        <span className="absolute inset-0 flex items-center justify-center font-sans text-[10px] [writing-mode:vertical-rl]">{bookProgress}%</span>
      </div>
    </nav>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-[min(88vw,390px)] overflow-y-auto border-r shadow-[18px_0_50px_rgba(0,0,0,.12)] ${panelClass}`}>
        <div className={`sticky top-0 z-10 border-b px-7 py-6 ${panelClass}`}>
          <div className="flex items-start justify-between gap-4">
            <div><p className="m-0 font-sans text-[11px] tracking-[.22em] text-muted">目录</p><h2 className="mb-1 mt-2 text-xl leading-snug">{bookTitle}</h2><p className="m-0 font-sans text-xs text-muted">{chapters.length} 章 · {characterCount.toLocaleString()} 字</p></div>
    ...
    <header className={`relative z-20 h-12 border-b backdrop-blur-md ${chromeClass}`}>
      <div className="mx-auto flex h-full max-w-[1180px] items-center gap-3 px-4 font-sans text-xs md:px-10">
        <Link to={backHref} className="text-current no-underline md:hidden" aria-label="返回问答">←</Link>
    ...
    {mode === "scroll" ? <div ref={scrollRef} onScroll={updateScrollProgress} onClick={handleInternalLink} className="h-[calc(100%-48px)] overflow-y-auto scroll-smooth">
      ...
      <article className={`relative mx-auto h-full max-w-[1180px] overflow-hidden border px-7 py-10 shadow-[0_16px_55px_rgba(32,32,28,.14)] sm:px-12 md:px-16 md:py-14 ${pageClass} ${theme === "dark" ? "border-[#2d312e]" : "border-[#d8d8d1]"}`}>
        ...
        <div ref={flowRef} data-book-page-flow onClick={handleInternalLink} className="h-full overflow-hidden [column-fill:auto] [&_figure]:break-inside-avoid [&_h1]:[break-after:avoid-column] [&_h2]:[break-after:avoid-column] [&_li]:break-inside-avoid" style={{ columnCount: columnsPerSpread, columnGap: columnsPerSpread === 2 ? "80px" : "48px", fontSize: `${fontSize}px`, lineHeight: 1.95 }}>
          {error && <p className="border-l-4 border-red bg-red/5 px-4 py-3 text-sm text-red">{error}</p>}{children}{chapterNavigation}
        </div>
        <button type="button" onClick={previousPage} disabled={!previousChapter && pageMetrics.page === 0} className="absolute inset-y-0 left-0 z-20 w-7 border-0 bg-transparent text-2xl text-muted opacity-0 cursor-pointer transition-opacity hover:opacity-100 focus-visible:opacity-100 disabled:hidden sm:w-11" aria-label="上一页">‹</button>
        <button type="button" onClick={nextPage} disabled={!nextChapter && pageMetrics.page >= pageMetrics.spreads - 1} className="absolute inset-y-0 right-0 z-20 w-7 border-0 bg-transparent text-2xl text-muted opacity-0 cursor-pointer transition-opacity hover:opacity-100 focus-visible:opacity-100 disabled:hidden sm:w-11" aria-label="下一页">›</button>
      </article>
      <div className="pointer-events-none absolute inset-x-0 bottom-1 flex items-center justify-center gap-4 font-sans text-[10px] text-muted md:bottom-2">
        <span>{firstPhysicalPage === lastPhysicalPage ? firstPhysicalPage : `${firstPhysicalPage}–${lastPhysicalPage}`} / {pageMetrics.physicalPages} 页</span><span>全书 {bookProgress}%</span>
      </div>
    </main>}
  </div>;
}

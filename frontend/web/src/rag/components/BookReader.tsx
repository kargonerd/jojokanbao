...
export function BookReader({
  ...
  useEffect(() => {
    ...
    if (continuous && !continuousReady) return;
    const frame = window.requestAnimationFrame(() => {
      const resume = resumePositionRef.current;
      const reader = scrollRef.current;
      if (!resume || !reader) return;
      if (continuous) continuousRef.current?.seek(activeChapterId, resume.progress * 100);
      else reader.scrollTo({ top: Math.max(0, reader.scrollHeight - reader.clientHeight) * resume.progress });
      setReadingProgress(resume.progress * 100);
      resumePositionRef.current = null;
    });
    return () => window.cancelAnimationFrame(frame);
  }, [activeChapterId, contentLoading, continuous, continuousReady, mode]);

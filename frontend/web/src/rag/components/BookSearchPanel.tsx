...
export function BookSearchPanel({ bookTitle, panelClass, onClose, onJump, onSearch }: BookSearchPanelProps) {
  ...
  return <aside aria-label="全书搜索" className={`fixed inset-y-0 right-0 z-50 flex w-full flex-col border-l shadow-[-18px_0_50px_rgba(0,0,0,.12)] sm:w-[min(88vw,430px)] ${panelClass}`}>
    <header className="border-b border-rule px-6 py-5">
      <div className="flex items-start justify-between gap-5"><div><p className="m-0 font-sans text-[11px] tracking-[.18em] text-red">全书搜索</p><h2 className="mb-0 mt-2 text-lg leading-snug">{bookTitle}</h2></div><button type="button" onClick={onClose} className="border-0 bg-transparent text-2xl text-current cursor-pointer" aria-label="关闭全书搜索">×</button></div>
      <form onSubmit={(event) => void submit(event)} className="mt-5 flex items-center gap-3 border-b border-rule focus-within:border-red">

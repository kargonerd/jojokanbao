...
export function SelectableAnnotationArticle({ subject, children }: { subject: AnnotationSubject; children: ReactNode }) {
  ...
  async function save(initialComment?: string) {
    if (!selection || saving) return;
    setNotice("");
    setSaving(true);
    try {
      const created = await annotations.create(selection.anchor, initialComment);
      clearSelection();
      setComment("");
    ...
    <>
      ...
      {selection ? (
        <div className="annotation-selection-tools" style={{ left: selection.left, top: selection.top }} role="toolbar" aria-label="选中文字工具">
          ...
          {commentOpen ? <section><textarea autoFocus value={comment} maxLength={2000} onChange={(event) => setComment(event.target.value)} placeholder="写下你的评论……" /><button type="button" disabled={saving || !comment.trim()} onClick={() => void save(comment.trim())}>{saving ? "保存中…" : "保存划线评论"}</button></section> : null}
        </div>
      ) : null}
      ...
      {active && currentUserId ? <AnnotationDiscussionPanel key={active.id}
        ...
        onComment={(body, parentCommentId) => annotations.comment(active.id, body, parentCommentId)}

...
export async function loadSpeechProviders(signal?: AbortSignal): Promise<SpeechCapabilities> {
  const response = await fetch("/api/v1/speech/providers", { signal });
...
export async function requestSpeech(
  ...
  const { useAccountSessionStore } = await import("../account/session");
  if (!useAccountSessionStore.getState().userId) throw new Error("请先登录后使用听读");
...
export async function speechKey(provider: string, version: string, voice: string, text: string): Promise<string> {
  const data = JSON.stringify([provider, version, voice, text.replace(/\s+/gu, " ").trim()]);
  const digest = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(data));
  return Array.from(new Uint8Array(digest), (value) => value.toString(16).padStart(2, "0")).join("");
}

function validateSpeechSource(value: unknown, cdn: string, key?: string): SpeechSource | null {

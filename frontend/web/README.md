...
首页“找书”只搜索已发布书籍的书名，在浏览器端完成模糊匹配并展示候选书目，由读者选择后打开；正文检索由 `/search` 提供，首次提交前可选择报刊或书籍。目录加载失败时可就地重试，搜索更新期间保留上次结果并显示局部加载状态。登录读者可从资料库加入“我的书架”，书架保存在账号的服务端数据中；继续阅读记录在本地保存，并在登录后按账号同步到 Web、桌面端和移动端。未登录点击加入书架会前往账号登录并带上 `returnTo`。资料库顶层直接展示报刊与书籍封面，不提供重复的标题、日期和二次搜索工具栏。
...
Web 不提供书籍离线下载，也不注册离线 Service Worker。共享书架的下载能力仅由 Desktop 入口启用。
登录同一账号后可同步 Safari、其他浏览器和原生客户端的阅读记录；
...
Desktop 和 Mobile 可在书架下载整本书，完整下载后可离线阅读；当前不提供系统推送。
...
本地全文检索通过 Vite 的 `/search-api` 同源代理访问现有搜索服务，支持任意开发端口，
...
每个账号对每条评论最多一个赞，重复请求不累加；私密想法和已隐藏内容不能点赞。
...
“本页标为已读”只提交当前已显示的未读消息 ID，每批最多 100 条，避免将刷新后
新到的消息一起标读。需要先应用 `202609080005_explicit_notification_read_batch.sql`；
它保留旧客户端接口，不新建通知表，也不授予读者直接更新表的权限。
...
找回密码时，正在验码的邮箱与已验码状态分开保存；只有验证码成功建立匹配的
账号会话后，才显示并允许提交新密码。验证码错误保留输入步骤，取消或切换账号
使本次恢复资格失效。此保护用于产品找回流程，不改变 Supabase 对有效登录会话
调用改密接口的服务端授权策略。

## 客户端书籍离线阅读

Desktop 复用书架和阅读器的离线能力，Web 不启用下载。使用方式和验证边界见
[`docs/offline-books.md`](../../docs/offline-books.md)。
阅读记录通过 `sync_reading_history` 同步，需先部署 `202609120001_reading_history_sync.sql`。
同一账号合并最近 16 条书籍／报刊记录，按最后阅读时间保留位置；章节内位置使用比例以适应不同屏幕。
本地写入后 1.5 秒内发起同步，前台恢复和联网时重试，并每 30 秒拉取其他设备的记录。
旧本地记录在首次登录时并入账号；退出／切换账号会隔离缓存。移动端清空记录会同步到账号，离线旧记录不会复活。

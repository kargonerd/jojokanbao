...
历史记录按登录账号保存在浏览器 IndexedDB，不设置自动过期，也不请求
`/gateway/conversations`。每轮请求只携带最近 20 条用户/助手消息，Agent 在客户端历史
模式下不读写 Makers Store。以后云同步只需同步同一套会话、消息和引用结构。单本提问
...
进程内 Store；国际 EdgeOne Makers Agent 只负责流式回答，Web 历史仍留在用户浏览器。

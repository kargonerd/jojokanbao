...
首页“找书”只搜索已发布书籍的书名，在浏览器端完成模糊匹配并展示候选书目，由读者选择后打开；正文检索由 `/search` 提供，首次提交前可选择报刊或书籍。目录加载失败时可就地重试，搜索更新期间保留上次结果并显示局部加载状态。登录读者可从资料库加入“我的书架”，书架保存在账号的服务端数据中；继续阅读记录仍保存在浏览器本地。未登录点击加入书架会前往账号登录并带上 `returnTo`。资料库顶层直接展示报刊与书籍封面，不提供重复的标题、日期和二次搜索工具栏。

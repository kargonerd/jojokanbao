...
- 当前正式开放：`/`、`/library`、`/search`、`/support`、`/account`、`/book/:datasetId/:itemKey`，以及 `/archive/*` 报刊阅读路由
- 待发布：`/rag/*`、`/olds/*`
...
未完成模块默认不可路由、不会显示在导航中，但源码仍会随项目进行类型检查和单元测试。本地开发某个模块时，将对应的 `VITE_ENABLE_*` 改为 `true` 并重启 Vite；正式部署在 rollout 前保持这些变量缺省或为 `false`。账号模块需要配置根目录 `.env.example` 列出的 Supabase 浏览器端公开值。

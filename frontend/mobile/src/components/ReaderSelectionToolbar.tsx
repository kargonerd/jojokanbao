...
export function ReaderSelectionToolbar({ selection, frame, theme, eInk, onCopy, onUnderline, onThought, onExplain, onClip }: {
  ...
  const position = positionReaderSelection(rect, { left: frame.x, top: frame.y, right: frame.x + frame.width, bottom: frame.y + frame.height }, { width: columns * 72, height: toolbarHeight });
  ...
  return <View style={[styles.container, { left: position.left, top: position.top, width: position.width }]}>
    ...
    <View pointerEvents="none" style={[styles.arrow, { left: position.arrowLeft - 7 }, position.above ? { top: toolbarHeight, borderTopWidth: 7, borderTopColor: backgroundColor } : { bottom: toolbarHeight, borderBottomWidth: 7, borderBottomColor: backgroundColor }]} />
  </View>;
}

const styles = StyleSheet.create({
  container: { position: "absolute", zIndex: 7 },
  actions: { padding: 4, flexDirection: "row", flexWrap: "wrap" },
  shadow: { shadowColor: "#000000", shadowOpacity: .18, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, elevation: 8 },
  action: { height: 52, alignItems: "center", justifyContent: "center", gap: 5 },

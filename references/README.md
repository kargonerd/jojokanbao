...
- `legacy-reader-web/`：来自 `C:\Users\luoxixi\WebstormProjects\web` 的旧 Vue reader 源码、主题、模板和 e2e/config 归档。当前可运行 reader 在 `apps/reader`，搜索服务在 `services/reader-search`，内部数据工作台在 `internal/data-workbench`。

...
def test_parse_source_import_package(monkeypatch):
    ...
    monkeypatch.setattr('app.rag.notebook_service.upload_source_asset', fake_upload_source_asset)
...
def test_parse_source_import_package_requires_chapter_path(monkeypatch):
    monkeypatch.setattr('app.rag.notebook_service.upload_source_asset', lambda *args: {
        'filename': args[2],
        'key': f'mock/{args[2]}',
...
def test_bind_source_import_package_saves_structured_profile(monkeypatch):
    ...
    monkeypatch.setattr('app.rag.notebook_service.cos_manager.upload_file', lambda *args, **kwargs: 'file:///tmp/source.md')
    monkeypatch.setattr('app.rag.notebook_service.SourceProfileManager.get', lambda notebook_id, source_id: saved_profiles[-1] if saved_profiles else base_profile)
    monkeypatch.setattr('app.rag.notebook_service.SourceProfileManager.save', lambda profile: saved_profiles.append(profile) or True)
...
def test_get_source_chapter_text_returns_selected_chapter(monkeypatch):
    monkeypatch.setattr('app.rag.notebook_service.SourceProfileManager.get', lambda notebook_id, source_id: {
        'chapters': [
...
def test_get_source_chapter_text_reports_missing_chapter(monkeypatch):
    monkeypatch.setattr('app.rag.notebook_service.SourceProfileManager.get', lambda notebook_id, source_id: {'chapters': []})

...
注册 API 使用 PostHog SDK 读取全局运行配置，按进程缓存已验证快照并后台刷新。
...
注册授权绑定邮箱、邀请码与短时有效期。批注不经后端：客户端用自己的会话直接调用
Supabase 批注 RPC，公开划线阈值固定在数据库内，身份和业务权限仍由数据库校验。
...
流式路径末尾的 `/` 不可省略：EdgeOne 生成的 ASGI 包装器会缓存无尾斜杠 GET 的全部响应再做路由探测。

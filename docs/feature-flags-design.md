...
`config` 不在数据库中枚举各 Flag 的字段，也不为单个配置建立专用读取函数。应用服务通过已有的管理读取接口获得完整配置；必须在数据库内强制执行的数值配置统一调用通用整数读取器，并由调用方传入路径、默认值和范围。

`reader.annotations` 当前使用 `config.publicMarkThreshold = 2`。划线查询通过通用读取器取得该值；配置缺失、类型错误或超出 `1..100` 时回退到 2。阈值不放在前端，避免绕过客户端后读取未达到公开条件的划线。

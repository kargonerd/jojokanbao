...
@dataclass
class AskStreamChunk:
    """A chunk of streaming response from ask_stream.

    Each chunk contains a portion of the answer text. The final chunk
    will have is_final=True and include the complete references.

    Attributes:
        text: The text content of this chunk (incremental).
        is_final: True if this is the final chunk with complete data.
        conversation_id: UUID for this conversation (available in final chunk).
        references: List of source references (available in final chunk).
    """

...
发布型 Capture 可复用 Runtime Bucket 的 `times/proxy/last-known-good.v1.json`：AES-256-GCM
加密，密钥由现有 `HF_TOKEN` 和订阅 URL 经 HKDF 派生，无需新增 secret。任何一项变更都会使旧缓存失效。
发布型 Capture 优先复用获取时间不足 12 小时的健康缓存，不请求订阅服务；满 12 小时后的第一轮采集
才重新下载。首次运行、缓存缺失/不可用、订阅 URL 或 HF Token 变更时立即下载；未启用缓存的手动
dry run 仍实时下载。只有实时下载且通过 Mihomo 本地代理健康检查的配置才可更新缓存；仅 master
发布型运行写入，始终覆盖同一个对象，内容不变也在满 12 小时后更新获取时间。
刷新遇到临时错误且重试耗尽时可使用获取时间不足 24 小时的旧缓存；401/403/404、证书或格式错误
仍使刷新失败。原始获取时间满 24 小时、时间异常或认证解密失败的缓存一律拒绝，复用和兜底均不会续期。
每次可选缓存读/写各有 20 秒总上限，缓存服务异常不会阻止可用的实时订阅。
运行摘要区分正常缓存命中与刷新失败后的降级兜底；
仍须通过代理健康检查和真实采集才报告业务成功。首次合并后由下一次健康的自然 Capture 自动建立缓存，
缓存未建立前不会伪装恢复。缓存及临时配置均不进入 Actions cache、Raw 或 artifact，也不涉及 SCF 部署。

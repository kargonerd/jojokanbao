...
只有实时下载且通过 Mihomo 本地代理健康检查的配置才可更新缓存；仅 master 发布型运行写入，
始终覆盖同一个对象。配置不变时最多每小时刷新一次，配置变更则立即更新。原始获取时间超过 24 小时、
时间异常或认证解密失败的缓存一律拒绝，缓存兜底不会续期。每次可选缓存读/写各有 20 秒总上限，
缓存服务异常不会阻止可用的实时订阅。订阅下载临时失败且重试耗尽时才读取缓存，使用情况写入运行摘要；

...
每 10 分钟 Capture
  发现栏目 URL
    ...
    → Playwright Chromium + BPC（需要时切换代理节点）
...
媒体之间并行；同一媒体始终串行并复用浏览器上下文、Cookie 和 BPC。代理轮换按全局回合进行，避免多个
并行媒体同时修改 Mihomo 路由。只有前一回合正文仍不完整的 URL 才进入下一节点。
...
已知无意义的直连。浏览器默认开启 JavaScript，使用持久上下文加载锁定 BPC。

...
function objects(value: unknown): Record<string, unknown>[] {
  return Array.isArray(value)
    ? value.map(object).filter((row): row is Record<string, unknown> => Boolean(row))
    : [];
}

function pagePromos(payload: unknown): Record<string, unknown>[] {
  ...
  return modules.flatMap((module) =>
    module.__typename === "PageListModule" ? objects(module.items) : []
  ).filter((item) => item.__typename === "PagePromo");
}

function candidate(source: SourceConfig, item: Record<string, unknown>): Candidate | undefined {
  ...
  try {
    canonicalUrl = normalizeArticleUrl(new URL(rawLink, AP_ROOT).href);
  } catch {
    return;
  }
  ...
  return {
    ...
    sourceUrl: new URL(rawLink, AP_ROOT).href,
    canonicalUrl,
...
export async function discoverAp(
  source: SourceConfig,
  endpoint: ApEndpoint,
  fetchedAt: string,
  ...
  url.searchParams.set("extensions", JSON.stringify({
    persistedQuery: { version: 1, sha256Hash: PERSISTED_QUERY },
  }));

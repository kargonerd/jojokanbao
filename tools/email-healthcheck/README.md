...
调度故障绑定到对应的计划槽位。该槽位已有真实工作流观察后，后续读取调度状态
或查询 GitHub 的错误不再触发邮件调度告警；延迟消费的同槽位观察也可解除此类
故障。上一槽位的观察不能解除新槽位的调度故障，采集、投递、SMTP 故障及漏跑
截止时间仍独立判断。
...
部署。需要动态调整的参数按 [运行配置约定](../../docs/feature-flags-design.md) 接入。

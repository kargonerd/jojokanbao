...
- API 请求失败、响应不合法或扫描不完整属于采集故障；空结果不能清除已有的
  投递或 SMTP 故障。
- 首次观察到 `failed` / `suppressed`，或邮件超过 10 分钟仍未投递时触发投递
  告警；近 30 分钟创建的邮件中至少 3 封 `bounced` 才触发退信告警。这里的
  “观察到”仍受 15 分钟轮询和随后 consumer 处理时间限制。

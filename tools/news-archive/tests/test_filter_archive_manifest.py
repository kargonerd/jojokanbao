...
def test_filter_manifest_collapses_caixin_pages_and_prefers_full_text(
    tmp_path: Path,
):
    source = tmp_path / "caixin.jsonl.gz"
    destination = tmp_path / "caixin-2010.jsonl.gz"
    base = "https://magazine.caixin.com/2010-02-07/100116568"
    rows = [
        {
            "publisher": "caixin",
            "canonicalUrl": base + suffix,
            "publishedAt": "2010-02-07T00:00:00Z",
            "candidates": [
                {
                    "provider": "wayback",
                    "snapshotUrl": (
                        "https://web.archive.org/web/20130101id_/"
                        "http://magazine.caixin.com/2010-02-07/"
                        "100116568" + suffix
                    ),
                    "digest": digest,
                }
    ...
    assert result["rowsSeen"] == 3
    assert result["rowsSelected"] == 1
    assert result["rowsCollapsed"] == 2
    assert selected[0]["canonicalUrl"] == base + ".html"
    assert [
        candidate["digest"] for candidate in selected[0]["candidates"]
    ] == ["FULL", "BASE", "PAGE"]

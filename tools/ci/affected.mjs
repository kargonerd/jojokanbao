...
const patterns = {
  ...
  node: /^(agent\/|frontend\/|infrastructure\/(cloudflare\/|tencent-scf\/maintenance-scheduler\/)|tools\/(ci\/|posthog\/|email-quota\/|monitoring\/|maintenance-scheduler\/|release\/|times-pipeline\/|[^/]+\/web\/)|package\.json$|pnpm-lock\.yaml$|pnpm-workspace\.yaml$|turbo\.json$|\.github\/workflows\/(check-email-quota|release-desktop|release-mobile(-eink|-ios|-ota)?|maintenance-times[^/]*|maintenance-sync-rmrb)\.yml$)/,

...
describe("Kindle PalmDOC decoding", () => {
  ...
  it("exports semantic inline images and controlled book layout to EPUB", async () => {
    ...
    try {
      ...
      expect($("img.jojo-inline-image").attr("src")).toBe("../assets/glyph.png");
      expect($("img.jojo-inline-image").attr("alt")).toBe("符号");
      expect($('figure[data-width="70"] figcaption').text()).toBe("图一");
      expect($('figure[data-role="cover"] img').attr("src")).toBe("../assets/glyph.png");
      expect($("img.jojo-inline-image")[0]?.next?.type).toBe("text");

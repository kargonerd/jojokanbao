...
export interface DispatchOptions {
  ...
  observedAtMs: number;
  slot: ScheduledSlot;
  beforePost?: (() => Promise<void>) | undefined;
  afterPost?: ((status: number) => Promise<void>) | undefined;
  reconcileOnly?: boolean | undefined;
}
...
export async function dispatchScheduledTask(
  ...
  // An ambiguous earlier delivery is reconciled by the run-list checks above.
  // Never blindly repeat an uncertain POST: GitHub has no idempotency-key API.
  if (options.reconcileOnly) throw new DispatchError("Dispatch outcome is still unconfirmed; refusing duplicate delivery", false);
  ...
  await options.afterPost?.(response.status);

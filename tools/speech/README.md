...
mini 播放器通过“原文”按钮定位听读位置，正文保持阅读器的常规样式。
Web/Desktop 和手机 WebView 共用 DOM Range 定位实现，使用正文字符偏移。
暂停后翻页时，听读进度独立保存，“原文”能返回当前听读位置。
从阅读位置起播时只裁开入口段，其后各段按缓存键复用音频。
...
`/api/v1/speech/providers?v=2` 提供逻辑音色目录，使用 MiMo 优先、Edge 回退协议。
未带版本参数的请求返回兼容目录，提供两种物理音色及对应缓存键；
两种目录均向用户展示男声、女声。
...
新闻前缀之外的共用目录包含书籍音频，清理范围限定为 `audio/speech/v1/news/`。
...
听读是 Web、Desktop、Android、iOS 和墨水屏 Android 的常规功能。
登录后可播放，退出登录时停止播放；客户端通过服务端声音列表读取合成能力。
播放优先复用 CDN 音频；MiMo 使用服务端密钥，自动模式在 MiMo 不可用时回退到 Edge。
...
全库入口使用独立离线流水线：缓存查验 → 合成/编码 → 本地持久化 → 独立上传 → 发布章节。音频落盘后释放合成名额。
...
章节仅在全部音频确认上传后发布；清单发布单独退避，合成按自身容量限制继续执行。STOP 会保存已在途合成、保留待上传队列后退出。
状态区分 `staged`/`stagedPerMinute5m`（本轮新合成落盘）、`generated`/`generatedPerMinute5m`（本轮确认上传）、`outbox.pending/pendingBytes`（跨重启待上传）及 `storage`（各存储阶段重试）。
...
所有线路不可用时执行退避和磁盘容量保护，保持 MiMo 出口和现有合成任务。
...
限流后每累计 32 个成功生成且没有新限流反馈的样本增加 8，直到恢复所设上限。鉴权故障和连续失败触发停机保护。
...
本机回环端口锁防止此工具重复启动。启动前确认同一批处理没有其他工具进程运行。
...
逐章工具的 checkpoint 可直接复用；尚未完整的章节按 B2 对象重新查缓存，已上传音频无需重新合成。
...
开发和生产遵循相同的服务商配置。密钥等配置在进程启动时读取，修改后需重启后端或重新部署。

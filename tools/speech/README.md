...
有句子时间戳时按实际媒体时间选择当前句子，变速和拖动不按字数估算；缺少时间戳时仍高亮当前音频段。
这是兼容行为，**不代表旧音频已经全部具备逐句同步数据**。本次没有重生成音频或执行全库补齐。

时间戳以 `<原始 MP3 内容 SHA256>.sentences-v1.json` 放在同一目录，独立于现有一年缓存的音频描述文件，
不改变音频缓存键。格式为 `jojo-speech-sentences/1`，包含 `audioSha256`、规范化朗读文本的 `textSha256`
及 `cues: [{start,end,startOffset,endOffset}]`。时间单位为秒，文字偏移按去掉空白后的 UTF-16 计数，
覆盖整个朗读文本。客户端拒绝音频/文本不匹配、不完整、倒序和越界的时间戳；加载失败不阻塞声音。
新启动时被裁开的入口段具有新文本/音频身份，也需要对应时间戳，不能套用原来整段的时间。

离线样本工具使用 FunASR `fa-zh` 给现有 MP3 补时间戳，默认只写本地 JSON。模型依赖与 API 部署隔离：

```powershell
python -m pip install -r tools/speech/requirements-alignment.txt
python tools/speech/align.py --audio <原始MP3> --text <该段原文UTF8文件> --output <音频sha256.sentences-v1.json>
```

该工具适用于普通话及可规范化的数字；不支持的英文/符号、字数不完整、时间倒序会报错，不发布猜测结果。
上传前应检查句子边界，并核对输出 `audioSha256` 与线上 MP3 文件名一致。工具不调用 TTS，不替换原音频，
不上传文件，不启动全库任务。全库补齐及新音频的持续对齐仍需单独安排运行资源和批处理流程。
